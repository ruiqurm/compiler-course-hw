...
enum STAGE
{
	...
	// 处理注释
	SLASH_COMMENT,
	SLASH_COMMENT_CONTENT,
	ASTERISK_COMMENT,
	ASTERISK_COMMENT_BS,
	ASTERISK_COMMENT_ESLASH,
...
shared_ptr<LexReport> parse(const string &code)
{
		...
		while (iter != code.end())
		{
			...
			case IDLE: // 初始状态
				...
				else if (ch == '.')
				{
					if ((iter + 1) != code.end() && isdigit(*(iter + 1)))
					{
						...
						result.push_back(Token{TOKEN_FLOAT_NUMBER, line, pos});
					}
					else
					{
						// .操作符
				...
				else if (ch == '\'')
				{
					// 字符
					state = CHAR_LITERAL;
					result.push_back(Token{TOKEN_CHAR_LITERAL, line, pos});
				}
				else
				{
					// do nothing
			...
			case NUMBER_INTEGER: // 数字整数
				...
				else if (ch == 'E' || ch == 'e')
				{
					...
					set_type(result, TOKEN_FLOAT_NUMBER);
				}
				else if (ch == '.')
				{
	...
	{
		if (token.type == TOKEN_IDENTITY)
		{
			auto tmp = tree.check(token.content);
			if (tmp != -1)
			{
				token.set_type(tmp);
			}
